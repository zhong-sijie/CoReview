...
export function useAuthState() {
  /** 当前登录状态，默认为false，表示未登录 */
  const [loggedIn, setLoggedIn] = useState(false);

  /**
   * 设置消息监听器和初始状态请求
   *
   * 在组件挂载时执行，负责建立与扩展端的通信桥梁。
   * 使用 useEffect 确保在组件生命周期内正确管理监听器。
   *
   * 执行流程：
   * 1. 注册鉴权状态消息的监听器
   * 2. 向扩展端请求当前鉴权状态
  ...
  useEffect(() => {
     ...
     * 使用双重否定确保loggedIn为布尔类型，处理可能的undefined值。
     *
     * @param message 来自扩展端的鉴权状态消息
     */
   ...
   * 返回鉴权状态
   *
   * 提供响应式的登录状态，当状态发生变化时，使用该hook的组件会自动重新渲染。
   *
   * @returns 包含登录状态的对象
   */
  return { loggedIn };
